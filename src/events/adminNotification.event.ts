...
class AdminNotificationService {
    ...
    private async getTrainingInfo(trainingId: string): Promise<any | null> {
        const [error, training] = await TrainingModel.byId({ id: trainingId })
    ...
    private async getAllAdmins(): Promise<AdminUser[]> {
        const [error, admins] = await AdminUserModel.all()
    ...
    public async notifySingleAdmin(
        ...
        try {
            const [error, admin] = await AdminUserModel.byId({ id: adminId })
